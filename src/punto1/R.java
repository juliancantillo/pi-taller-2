/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package punto1;

/**
 *
 * @author invitado
 */
public class R {
    public static String
            TITLE = "Chat Programación Interactiva",
            SEND_BUTTON = "Enviar",
            INPUT_PLACEHOLDER = "Digite su mensaje aquí",
            LOGIN_BUTTON = "Ingresar al Chat",
            CLEAN_BUTTON = "Limpiar Chat",
            LOGOUT_BUTTON = "Salir del Chat",
            OPTIONS = "Opciones",
            CONNECTED_USERS = "Usuarios Conectados",
            NEW_USERNAME = "Ingrese Nombre y Apellido",
            NOT_EMPTY_MSG_ALLOWED = "No se pueden enviar mensajes vacíos",
            MUST_BE_LOGED_IN = "Debe ingresar primero"
            ;
}
